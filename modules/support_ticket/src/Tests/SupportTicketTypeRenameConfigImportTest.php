...
/**
 * @file
 * Contains \Drupal\support_ticket\Tests\SupportTicketTypeRenameConfigImportTest.
 */
...
/**
 * Tests importing renamed support_ticket type via configuration synchronization.
 *
 * @group support_ticket
 */
class SupportTicketTypeRenameConfigImportTest extends SupportTicketTestBase {
   ...
   * @var array
   */
  public static $modules = array('support_ticket', 'text', 'config');
  ...
  public function testConfigurationRename() {
    $content_type = $this->supportTicketCreateSupportTicketType(array(
      'type' => Unicode::strtolower($this->randomMachineName(16)),
      'name' => $this->randomMachineName(),
    ));
    $staged_type = $content_type->id();

    // Check the default status value for a support_ticket of this type.
    $support_ticket = entity_create('support_ticket', array('support_ticket_type' => $staged_type));
    $this->assertTrue($support_ticket->status->value, 'SupportTicket status defaults to TRUE.');
    ...
    $support_ticket = entity_create('support_ticket', array('support_ticket_type' => $active_type));
    $this->assertFalse($support_ticket->status->value, 'SupportTicket status defaults to FALSE.');
    ...
    // rename five configuration entities: the support_ticket type, the body field
    // instance, two entity form displays, and the entity view display.
    // @see \Drupal\support_ticket\Entity\SupportTicketType::postSave()
    $expected = array(
      'support_ticket.type.' . $active_type . '::support_ticket.type.' . $staged_type,
      'core.base_field_override.support_ticket.' . $active_type . '.status::core.base_field_override.support_ticket.' . $staged_type . '.status',
      'core.entity_form_display.support_ticket.' . $active_type . '.default::core.entity_form_display.support_ticket.' . $staged_type . '.default',
      'core.entity_view_display.support_ticket.' . $active_type . '.default::core.entity_view_display.support_ticket.' . $staged_type . '.default',
      'field.field.support_ticket.' . $active_type . '.body::field.field.support_ticket.' . $staged_type . '.body',
    );
    ...
    $this->assertFalse(SupportTicketType::load($active_type), 'The content no longer exists with the old name.');
    $content_type = SupportTicketType::load($staged_type);
    if (isset($content_type)) {
      $this->assertIdentical($staged_type, $content_type->id());
    }
    else {
      $this->error("Couldn't load support ticket type.");
    }
